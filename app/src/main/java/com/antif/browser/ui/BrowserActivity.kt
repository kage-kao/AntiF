...
class BrowserActivity : AppCompatActivity() {
    ...
    private fun showBrowserMenu(anchor: View) {
        PopupMenu(this, anchor).apply {
            ...
            menu.add(0, 3, 0, "🗑️ Close other tabs")
            
            // Manual Emergent Bypass Actions (simplified)
            menu.add(1, 50, 0, "--- Quick Bypass ---")
            menu.add(1, 56, 0, "⚡ Run FULL BYPASS Now")
            menu.add(1, 51, 0, "🔄 Reset ALL modals")
            menu.add(1, 52, 0, "🧹 Full cleanup")
            menu.add(1, 55, 0, "🆔 New Device-Id")
            
            // Regional Discounts (only Turkey and Canada - FREE)
            menu.add(2, 60, 0, "--- Regional ---")
            menu.add(2, 61, 0, "🇹🇷 Turkey (Free)")
            menu.add(2, 62, 0, "🇨🇦 Canada (Free)")
            
            // AdBlock
            menu.add(3, 10, 0, "--- AdBlock ---")
            menu.add(3, 4, 0, if (adblockEnabled) "🛡️ AdBlock: ON" else "⚠️ AdBlock: OFF")
            menu.add(3, 5, 0, "📊 AdBlock stats")
            
            // Privacy
            menu.add(4, 20, 0, "--- Privacy ---")
            menu.add(4, 6, 0, "🍪 Clear cookies")
            menu.add(4, 7, 0, "🗑️ Clear all data")
            menu.add(4, 9, 0, "🔧 Cookie manager")
            
            // Tools
            menu.add(5, 30, 0, "--- Tools ---")
            ...
            setOnMenuItemClickListener { item ->
                when (item.itemId) {
                    ...
                    // Manual Emergent Bypass Actions
                    56 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateFullBypassScript(), null)
                        Toast.makeText(this@BrowserActivity, "⚡ FULL BYPASS ACTIVATED!", Toast.LENGTH_LONG).show()
                        log("MANUAL", getActiveWebView()?.url ?: "", "FULL BYPASS manually executed")
                    }
                    51 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateFullResetModalsScript(), null)
                        Toast.makeText(this@BrowserActivity, "All Emergent modals reset!", Toast.LENGTH_SHORT).show()
                        log("MANUAL", getActiveWebView()?.url ?: "", "Reset modals manually executed")
                    }
                    52 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateFullCleanupScript(), null)
                        Toast.makeText(this@BrowserActivity, "Full Emergent cleanup done!", Toast.LENGTH_SHORT).show()
                        log("MANUAL", getActiveWebView()?.url ?: "", "Full cleanup manually executed")
                    }
                    55 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateDeviceIdSpoofScript(), null)
                        Toast.makeText(this@BrowserActivity, "New Device-Id generated!", Toast.LENGTH_SHORT).show()
                        log("MANUAL", getActiveWebView()?.url ?: "", "New Device-Id manually executed")
                    }
                    
                    // Regional Discounts (only Turkey and Canada)
                    61 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateRegionalDiscountScript("turkey"), null)
                        Toast.makeText(this@BrowserActivity, "🇹🇷 Turkey settings applied! Reload page.", Toast.LENGTH_LONG).show()
                    }
                    62 -> {
                        getActiveWebView()?.evaluateJavascript(EmergentBypass.generateRegionalDiscountScript("canada"), null)
                        Toast.makeText(this@BrowserActivity, "🇨🇦 Canada settings applied! Reload page.", Toast.LENGTH_LONG).show()
                    }
