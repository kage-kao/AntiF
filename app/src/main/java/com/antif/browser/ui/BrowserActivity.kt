...
class BrowserActivity : AppCompatActivity() {
    ...
    // Track bypass application per domain (prevent spam)
    private val bypassAppliedDomains = mutableSetOf<String>()
    ...
    private fun configureWebView(webView: WebView, p: BrowserProfile) {
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                // Inject fingerprint spoofing script (always)
                ...
                // FIXED: Only apply bypass ONCE per domain per session
                if (shouldApplyBypass && domain.isNotEmpty() && !bypassAppliedDomains.contains(domain)) {
                    bypassAppliedDomains.add(domain)
    ...
    private fun setupNavButtons() {
        btnRefresh.setOnClickListener { 
            // Clear bypass flag for current domain on manual refresh
            val currentUrl = getActiveWebView()?.url
            val domain = extractDomain(currentUrl)
            bypassAppliedDomains.remove(domain)
            ...
            setOnMenuItemClickListener { item ->
                when (item.itemId) {
                    100 -> {
                        ...
                        // Clear applied domains to allow re-application
                        bypassAppliedDomains.clear()
